...
evaluation with no test coverage.
It gates `just rfd-promote` and runs inside every `just rfd-cycle` round, so a
wrong answer changes what gets written.
...
- Sentence-shaped rules counted wrapped lines rather than sentences, because
  `comfort` wraps at 80 columns.
  RFD 001's three-sentence Summary reported as six, a paragraph reported 13
  sentences, and `duplicate` silently compared line fragments and so never
  fired.
  Fixed by joining consecutive prose lines into blocks first.
...
The project already runs node checks in the website workflow.
A sibling `rfd-lint.test.mjs` using `node:test` keeps the dependency count at
zero, and wires into `docs-ci` next to `rfd-lint-ci`.

## Prior art

A parallel effort on this problem shipped node checker tests as part of its CI
and verified them before landing.
This is the piece of that work worth copying outright.
