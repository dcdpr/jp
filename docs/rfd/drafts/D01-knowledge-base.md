...
- **Wasm execution.** The v1 implementation runs `jp_tool_learn` as native
  Rust. Migration to Wasm is deferred to a later phase.

  > [!TIP]
  > [RFD 016](016-wasm-plugin-architecture.md) defines the Wasm plugin
  > infrastructure that `learn` would migrate to, with
  > [RFD 017](017-wasm-attachment-handlers.md) covering the first capability
