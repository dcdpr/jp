...
use jp_plugin::message::{
    ConfigEntry, ConversationSummary, DraftResponse, EventsResponse, ExitMessage, HostToPlugin,
    InterruptRequest, OptionalId, PluginToHost, QueryRequest, ReadDraftRequest, ReadEventsRequest,
    WriteDraftRequest,
};
...
impl PluginClient {
    ...
    pub async fn query(&self, conversation: &str, content: &str) -> Result<(), ClientError> {
        let id = self.next_id();
    ...
    pub async fn read_draft(&self, conversation: &str) -> Result<DraftResponse, ClientError> {
        let id = self.next_id();
        let msg = PluginToHost::ReadDraft(ReadDraftRequest {
            id: Some(id.clone()),
            conversation: conversation.to_owned(),
        });
