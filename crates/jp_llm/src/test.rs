...
use jp_config::{
    ...
    model::{
        id::{ModelIdConfig, PartialModelIdConfig, PartialModelIdOrAliasConfig, ProviderId},
...
impl TestRequest {
    ...
    pub fn model(mut self, model: ModelIdConfig) -> Self {
        ...
        match &mut self {
            ...
            Self::ModelDetails { .. } | Self::ToolCallResponse { .. } | Self::Func(_) => {}
        }
    ...
    pub fn assert_chat(mut self, assert: impl Fn(&[Vec<Event>]) + 'static) -> Self {
        match &mut self {
            Self::Chat { assert: a, .. } => *a = Arc::new(assert),
            Self::Structured { .. }
            | Self::ModelDetails { .. }
            | Self::ToolCallResponse { .. }
            | Self::Func(_) => {}
    ...
    pub fn assert_structured(
        ...
        match &mut self {
            Self::Structured { assert: a, .. } => *a = Arc::new(assert),
            Self::Chat { .. }
            | Self::ModelDetails { .. }
            | Self::ToolCallResponse { .. }
            | Self::Func(_) => {}
    ...
    pub fn assert_models(mut self, assert: impl Fn(&[ModelDetails]) + 'static) -> Self {
        match &mut self {
            Self::ModelDetails { assert: a, .. } => *a = Arc::new(assert),
            Self::Structured { .. }
            | Self::Chat { .. }
            | Self::ToolCallResponse { .. }
            | Self::Func(_) => {}
    ...
    pub fn as_thread(&self) -> Option<&Thread> {
        match self {
            ...
            Self::ModelDetails { .. } | Self::ToolCallResponse { .. } | Self::Func(_) => None,
        }
    }
        ...
        |recording, url| async move {
            ...
            for (index, mut request) in requests.into_iter().enumerate() {
                ...
                let config = match &request {
                    ...
                    TestRequest::ModelDetails { .. } => PartialAppConfig::empty(),
                ...
                match request {
                    ...
                    TestRequest::ModelDetails { assert } => {
                        let value = provider.models().await.unwrap();
                        model_details.extend(value);
