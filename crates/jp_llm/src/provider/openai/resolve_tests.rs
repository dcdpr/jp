...
use jp_credentials::{
    CredentialBackend, DEFAULT_COOLDOWN, InMemoryCredentialBackend, MAX_COOLDOWN,
};
...
async fn test_an_entry_is_tried_once_per_request() {
    ...
    let second = advance(&chain, None, &first, &refused, "gpt-5.6", now(), false)
        .await
        .unwrap();
    assert_eq!(
        second.selected,
    ...
    let third = advance(&chain, None, &second, &refused, "gpt-5.6", now(), false).await;
