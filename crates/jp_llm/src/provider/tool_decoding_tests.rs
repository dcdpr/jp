...
use crate::{
    EventStream, decoding::ArgumentDecoders, event::Event, event_builder::EventBuilder,
    model::ModelDetails, query::ChatQuery,
};
...
async fn provider_round_trip(provider_id: ProviderId, body: &'static str, streaming: bool) {
    ...
    while let Some(event) = events.next().await {
        match event.unwrap() {
            ...
            // Bookkeeping that carries no tool call arguments of its own.
            Event::Finished(_)
            | Event::KeepAlive
            | Event::Notice(_)
...
#[tokio::test]
async fn vllm_decodes_before_tool_dispatch() {
    provider_round_trip(ProviderId::Vllm, CHAT_CALL, true).await;
}
