...
/// runs `jp conversation compact` or uses `--compact`.
/// Each rule produces one compaction event in the conversation stream.
...
impl FillDefaults for PartialCompactionConfig {
    fn fill_from(self, defaults: Self) -> Self {
        ...
        // `RuleBound::default()` (0), which would compact the whole conversation.
        let rule_defaults = PartialCompactionRuleConfig::default_values(&())
            .ok()
            .flatten()
            .unwrap_or_default();
