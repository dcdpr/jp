...
impl KvAssignment {
    ...
    /// Try to parse the value as a boolean via [`From<bool>`], or as a string
    /// via [`FromStr`].
    pub(crate) fn try_bool_or_from_str<T, E>(self) -> Result<T, KvAssignmentError>
    where
    ...
    /// Convenience method for [`Self::try_bool_or_from_str`] that wraps the
    /// `Ok` value into `Some`.
    pub(crate) fn try_some_bool_or_from_str<T, E>(self) -> Result<Option<T>, KvAssignmentError>
    where
        T: From<bool> + FromStr<Err = E>,
        E: Into<BoxedError>,
    {
