...
async fn static_rules_resolve_without_running_anything() {
    ...
    let resolver = Resolver::new(&rules, tmp.path(), false, &printer);
...
async fn automatic_application_drops_a_failing_command() {
    let (rules, tmp, printer, err) = setup(
        r#"{
            "ok": "kept",
...
async fn alias_errors_on_an_unknown_name() {
    let (rules, tmp, printer, _err) = setup(r#"{ "known": "x" }"#);
    let resolver = Resolver::new(&rules, tmp.path(), false, &printer);
...
async fn ask_without_a_terminal_aborts() {
    let (rules, tmp, printer, _err) = setup(r#"{ "asks": { "value": { "cmd": "echo x" } } }"#);
    let resolver = Resolver::new(&rules, tmp.path(), false, &printer);
...
async fn static_rules_are_unaffected_by_run_policy() {
    let (rules, tmp, printer, _err) = setup(r#"{ "plain": { "value": "v", "run": "ask" } }"#);
    let resolver = Resolver::new(&rules, tmp.path(), false, &printer);
