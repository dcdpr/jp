...
use crate::{
    ...
    ObjectType, SchemaType, Schematic, StringType, StructType, TupleType, UnionType,
};
    ...
    pub fn partialize(&mut self) {
        match &mut self.ty {
            ...
            SchemaType::Union(inner) => {
                ...
                if is_nullable {
                    for item in &mut inner.variants_types {
                        if !item.is_null() {
                            item.partialize();
                        }
                    }
...
impl fmt::Display for Schema {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        if let Some(name) = &self.name
            && (self.ty.is_struct() || self.ty.is_reference())
        {
            write!(f, "{name}")
        } else {
            write!(f, "{}", self.ty)
...
/// Returns `true` if the value is equal to the type's default.
///
/// Used with serde's `skip_serializing_if` to elide default-valued fields.
/// The function is generic so the trivially-copy-by-ref lint does not fire on
/// `bool` arguments — serde's contract requires the parameter to be `&T`.
#[cfg(feature = "serde")]
