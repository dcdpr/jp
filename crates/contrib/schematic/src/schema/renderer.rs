...
/// Error returned from schema rendering and generation.
#[derive(Debug, Error)]
pub enum RenderError {
    /// I/O error while writing the rendered output.
    #[error(transparent)]
    Io(#[from] std::io::Error),

    /// Catch-all rendering failure with a free-form message.
    #[error("{0}")]
    Custom(String),
}

impl RenderError {
    /// Construct a [`RenderError::Custom`] from any displayable value.
...
pub trait SchemaRenderer<O = String> {
    ...
    fn render_schema_without_reference(&mut self, schema: &Schema) -> RenderResult<O> {
        match &schema.ty {
            ...
            SchemaType::Reference(reference) => self.render_reference(&reference.name, schema),
        }
    }
