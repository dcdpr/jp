...
ticket-labels-sync:
    ...
    addable=$(tokens false)
    keepable=$(tokens true)

    {
        ...
        echo '#'
        echo '# `ticket_create` is offered only the values it may add. `ticket_label` and'
        echo '# `ticket_list` also get retired ones: replacing a whole set means naming a'
        ...
        for tool in ticket_label ticket_list; do
            echo
            echo "# Includes retired values: see the note above."
            echo "[conversation.tools.${tool}.parameters.labels.items]"
            echo 'enum = ['
            echo "$keepable"
            echo ']'
        done
