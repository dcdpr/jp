...
selected conversations copy three URIs and open three windows.
The list binds a single `String?`, so it never holds more than one conversation:
a shift-click does not extend the selection, and Copy Link on three rows copies
one URI.
