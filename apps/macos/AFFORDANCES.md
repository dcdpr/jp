...
- **Restored per window**: the workspace, the selected conversation, the
  ...
  showing.
- **One window per workspace**, keyed by the workspace path.
  Opening the same workspace twice reaches the same window rather than making a
  second one, which is why the path is canonicalized before it is used as the
  key.
